...
#' @seealso
#'
#' \code{\link{clcep}}, \code{\link{harmonicfit}}, \code{\link{IARloglik}}, \code{\link{CIARkalman}},\code{\link{IARPermutation}}
#'
#' @examples
#' data(clcep)
#' f1=0.060033386
...
#' test
IARTest<-function (y, st, merr=0,f, phi,model="iAR",plot = TRUE, xlim = c(-1, 0),nu=3)
{
  ...
  for (j in 1:l1) {
    ...
    if(model=="iAR-Gamma")
    {
      ...
      if(min(y1)<0)
        y1=y1-min(y1)
      bad[j] = IARgamma(y1, st=st)$phi
    }
    if(model=="CiAR")
    {
      res3 = CIARkalman(y=y, t=st,delta=merr)[1]
      bad[j] = res3$phiR
    }
  }
